                        ...
                        <tbody>
                            <c:forEach var="c" items="${customers}">
                                <tr>
                                    ...
                                    <td>${c.address}</td>
                                    <td>${c.phoneNumber}</td>
                                    <td><a href="/Wellness/admin/customers/update/${c.customerId}" class="update btn btn-warning btn-sm"></a>
                                        <c:if test="${c.customerId==1}">
                                        </td><td><a id="deletebtn" href="/Wellness/admin/customers/delete/${c.customerId}" class="delete btn btn-danger btn-sm disabled"></a></td>
                                        </c:if>
                                        <c:if test="${c.customerId!=1}">
                                    </td><td><button href="/Wellness/admin/customers/delete/${c.customerId}" class="delete btn btn-danger btn-sm"></button></td>
                                </c:if>
                        </tr>
                    </c:forEach>
                </tbody>
